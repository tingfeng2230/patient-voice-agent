...
from utils.audio_helper import save_audio_from_base64, cleanup_audio_file
...
async def process_audio(request: Request):
    ...
    接收Base64编码的音频数据，上传到对象存储后调用Agent进行语音识别和患者信息提取
    """
    ctx = new_context(method="process_audio", headers=request.headers)
    request_context.set(ctx)
    ...
    try:
        ...
        logger.info(f"Received audio processing request: run_id={ctx.run_id}, format={audio_format}")
        
        # 保存Base64音频到临时文件
        filepath, _ = save_audio_from_base64(audio_base64, audio_format)
        logger.info(f"Audio saved to temporary file: {filepath}")
        
        try:
            # 上传到对象存储
            file_key = storage.stream_upload_file(
                fileobj=open(filepath, 'rb'),
                file_name=f"patient_audio_{ctx.run_id}.{audio_format}",
                content_type=f"audio/{audio_format}"
            )
            logger.info(f"Audio uploaded to storage with key: {file_key}")
            
            # 生成预签名URL（有效期1小时）
            audio_url = storage.generate_presigned_url(key=file_key, expire_time=3600)
            logger.info(f"Generated presigned URL for audio")
            
            # 调用Agent处理音频URL
            agent_input = {
                "type": "query",
                "session_id": ctx.run_id,
                "message": f"请帮我处理这段录音并提取患者信息。音频URL：{audio_url}",
                "content": {
                    "query": {
                        "prompt": [
                            {
                                "type": "text",
                                "content": {"text": f"请帮我处理这段录音并提取患者信息。音频文件URL：{audio_url}"}
                            }
                        ]
                    }
                }
            ...
            # 调用Agent
            result = await service.run(agent_input, ctx)
            ...
            return {
                "status": "error",
                "message": "Failed to extract patient information",
                "run_id": ctx.run_id
            }
        ...
        logger.error(f"JSON decode error in process_audio: {e}")
        raise HTTPException(status_code=400, detail="Invalid JSON format")
    except Exception as e:
...
class ConnectionManager:
    def __init__(self):
        # 存储所有活跃的 WebSocket 连接：{session_id: websocket}
    ...
    def update_session_data(self, session_id: str, data: Dict[str, str]):
        if session_id not in self.session_data:
            self.session_data[session_id] = {
                "patient_name": "",
                "medical_record": ""
    ...
    def get_session_data(self, session_id: str) -> Dict[str, str]:
        return self.session_data.get(session_id, {"patient_name": "", "medical_record": ""})
...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    ...
    try:
        # 发送当前会话的历史数据
        current_data = manager.get_session_data(session_id)
        ...
        while True:
            ...
            if data.get("type") == "update":
                ...
                manager.update_session_data(session_id, update_data)
